...
import CaseDetailModal from "@/app/components/CaseDetailModal";
import {
  fetchCaseData,
...
interface EvidenceData {
  ...
  analyzed: boolean;
  files: UploadedFile[];
  status: "ready" | "needs-improvement" | "none"; // "ready" = all valid, "needs-improvement" = some invalid, "none" = not analyzed
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchEvidenceRecommendations(USER_ID)
      .then((recs) => {
  ...
  const handleStartPractice = () => {
    // Navigate to front_end_3 (court simulator) on port 3001 with user context
    window.location.href = `http://localhost:3001?user_id=${USER_ID}&case_id=1`;
  };

  const handleEvidenceUpdate = useCallback((index: number, hasFiles: boolean, analyzed: boolean, files: UploadedFile[], status: "ready" | "needs-improvement" | "none") => {
  ...
  const totalFilesUploaded = Object.values(evidenceStates).reduce((sum, state) => sum + (state?.files?.length || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      {showCaseDetailModal && (
        <CaseDetailModal isOpen={showCaseDetailModal} onClose={handleCloseCaseDetailModal} caseData={caseData} />
      )}
    </div>
  );
}
