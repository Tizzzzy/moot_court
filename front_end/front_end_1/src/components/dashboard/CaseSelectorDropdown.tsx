...
export function CaseSelectorDropdown({
  ...
  const getCaseLabel = (c: DashboardCase) => {
    const plaintiff = c.plaintiffs?.[0]?.name;
    const defendant = c.defendants?.[0]?.name;
    if (plaintiff || defendant) {
      const parties = `${plaintiff ?? 'Unknown'} vs ${defendant ?? 'Unknown'}`;
      return c.hearing_date ? `${parties} (${c.hearing_date})` : parties;
    }
    return c.alias || c.case_number || 'Unnamed';
  };
