...
export function HearingOverview({ onStartHearing, caseData }: HearingOverviewProps) {
  return (
    <div className="min-h-screen bg-[#f9fafb]">
      ...
      <div className="bg-white border-b border-[rgba(0,0,0,0.1)] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_0px_rgba(0,0,0,0.1)]">
        <div className="max-w-[1344px] mx-auto px-12 py-4">
          <button className="flex gap-3 items-center px-3 py-1.5 rounded-lg mb-2 hover:bg-[#f8fafc] transition-colors">
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#e2e8f0] shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex">
                ...
                <span className="text-[15px] text-[#64748b]">{caseData?.case_number || 'SC-12345'}</span>
              </div>
              <div className="flex">
                <span className="font-semibold text-[15px] text-[#475569] min-w-[140px]">Plaintiff:</span>
                <span className="text-[15px] text-[#64748b]">{caseData?.plaintiffs?.[0]?.name || 'Plaintiff'}</span>
              </div>
              <div className="flex">
                <span className="font-semibold text-[15px] text-[#475569] min-w-[140px]">Defendant:</span>
                <span className="text-[15px] text-[#64748b]">{caseData?.defendants?.[0]?.name || 'Defendant'}</span>
              </div>
              <div className="flex">
                <span className="font-semibold text-[15px] text-[#475569] min-w-[140px]">Claim:</span>
                <span className="text-[15px] text-[#64748b]">{caseData?.claim_summary || 'Plaintiff seeks compensation for damages.'}</span>
              </div>
            </div>
          </div>
